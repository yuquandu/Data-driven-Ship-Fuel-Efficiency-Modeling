...
# Loading data
path_rep  = './data_management/Ship_S3_AIS_Weather_Combination.xlsx'
path_hyp  = './data_para/Ship_S3_AIS_Weather_Combination_Para.xlsx'
...
x = np.array(Train_data.iloc[:, 0:10])                                   # Load variable data
...
for i in range(1,21):
    ...
    rfr = GradientBoostingRegressor(n_estimators = int(best_n[i-1]), 
                                    max_features = None,
                                    # max_features = int(best_features[i-1]), 
                                    learning_rate = float(best_rate[i-1]), random_state = 7, max_depth = int(best_depth[i-1]), 
                                    subsample = float(best_subsample[i-1]), min_samples_leaf = int(best_leaf[i-1]), 
                                    min_samples_split = int(best_split[i-1]))
    # train
    rfr.fit(x_train, y_train)                                              # Change y_train to 1 line, regression training
    # save model to file
    model_name = path_save + filename + '_GB_' + str(i)
    print(model_name)
    ...
    # Evaluation of Gradient boost regression models
...
GB_Result['Random_seed'] = random_seed
GB_Result['Train ACC'] = E_0_Train
GB_Result['Test ACC'] = E_0_Test
GB_Result['Train R2'] = E_1_Train
GB_Result['Test R2'] = E_1_Test
GB_Result['Train MSE'] = E_2_Train
GB_Result['Test MSE'] = E_2_Test
GB_Result['Train RMSE'] = E_3_Train
GB_Result['Test RMSE'] = E_3_Test
GB_Result['Train MAE'] = E_4_Train
GB_Result['Test MAE'] = E_4_Test
GB_Result['Train EVS'] = E_5_Train
GB_Result['Test EVS'] = E_5_Test
GB_Result['Train MAPE'] = E_6_Train
GB_Result['Test MAPE'] = E_6_Test
GB_Result['A_R2'] = E_7
GB_Result['run time'] = time_list
GB_importance = pd.DataFrame(impor)
GB_importance.columns = Train_data.columns.values[0:10]


GB_Result.to_excel(path_save+filename+'_GB_Result.xlsx', index=False) 
GB_importance.to_excel(path_save+filename+'_GB_Impor.xlsx', index=False)
print('Results saved')
